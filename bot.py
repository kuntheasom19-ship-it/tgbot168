...
from telegram import ReplyKeyboardMarkup, InlineKeyboardButton, InlineKeyboardMarkup, Update, BotCommand, MenuButtonCommands
...
def get_sub_menu_emoji(sub_name):
    if sub_name.endswith('_CH_KH'): return '🌟'
    if sub_name.endswith('_V_KH'): return '✨'
    if sub_name.endswith('_PK_KH'): return '🍀'
    if sub_name.endswith('_TH_KH'): return '🎯'
    if sub_name.endswith('_KH'): return '⭐'
    if '_CH' in sub_name: return '🔥'
    if '_V' in sub_name: return '👑'
    if '_TH' in sub_name: return '💎'
    if '_PK' in sub_name: return '🛡️'
    if '_KL' in sub_name: return '🏆'
    if 'Joker' in sub_name: return '🃏'
    if 'BK' in sub_name: return '👑'
    if 'MB' in sub_name: return '💎'
    if sub_name.startswith('Ball'): return '⚽'
    if sub_name.startswith('F88'): return '🔥'
    if sub_name.startswith('Makav'): return '💰'
    if sub_name.startswith('Wa855'): return '🎲'
    if sub_name.startswith('M99'): return '🏆'
    if sub_name.startswith('Betwos'): return '💎'
    if sub_name.startswith('SB'): return '⚡'
...
async def handle_menu_click(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if row:
            ...
            cursor.execute("""
                INSERT INTO history (user_id, tg_username, tg_name, menu_name, account_username, account_password, timestamp, stock_left)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
...
async def post_init(application: Application) -> None:
    try:
        await application.bot.set_my_commands([
            BotCommand("start", "សូមចាប់ផ្តើម និងបង្ហាញម៉ឺនុយ")
        ])
        await application.bot.set_chat_menu_button(menu_button=MenuButtonCommands())
        logging.info("Successfully set bot commands & chat menu button.")
    except Exception as e:
